...

{
  ...
  "categories": ["AFFILIATE_MARKETING", "PUBLICIDAD","REDES SOCIALES"],
  "brand": {
    "id": "sidn-datalitica",
    "displayName": "gtm-sidn-datalitica",
...
const trackEvent = function(){
  if (eventName=='PageView'){
      ...
      log('TikTok| tracking '+eventName);
  } else {
    if (eventParams){
      callInWindow('ttq.track', eventName, eventParams);
      log('TikTok| tracking '+eventName+' with params:', eventParams);
    } else {
...
if (isConsentGranted('ad_storage')){
  injectScript(url,sendEvent,data.gtmOnFailure);
  sent = true;
} else {
