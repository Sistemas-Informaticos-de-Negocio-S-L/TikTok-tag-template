...

{
  ...
  "description": "Tag to implement TikTok pixel",
...
[
  {
    ...
    "help": "If using multiple IDs, please mind that all events will be sent to all pixels",
...
data.library_url="https://analytics.tiktok.com/i18n/pixel/sdk.js?sdkid="; // TikTok library

// Load a pixel and runs the sendEvent function
const loadPixelAndSendEvent = (pixel_id, eventName, eventParams) => {
  injectScript(
    data.library_url+pixel_id,
    ()=>{if(eventName != 'Pageview') {sendEvent(pixel_id, eventName, eventParams);}},
    data.gtmOnFailure
  );
};

// Send the event using ttq.track function
const sendEvent = (pixel, eventName, eventParams) => {
  callInWindow('ttq.track',eventName, eventParams, {pixel_code:pixel});
};
...
const main = () => {
  ...
  tag_pixels.forEach((id) => {
    if (ttq && ttq._i[id] && ttq._i[id]._init){
      sendEvent(id,eventName,eventParams);
    } else {
      loadPixelAndSendEvent(id,eventName,eventParams); 
    }
  });
};
